...
import urlparse, { pathToFileURL } from "url";
...
class BaseApplication {
  ...
  constructor(app: App) {
    ...
    // @ts-expect-error - Electron IPC callback parameter type mismatch
    ...
    // @ts-expect-error - Electron IPC callback parameter type mismatch
    ipcMain.on("browser:progress", (progress: number) =>
  ...
  private onSignIn(event: Electron.Event, args: any) {
    log.info("Sign in event received", args);
  }

  private onSignedIn(event: Electron.Event, args: any) {
    log.info("Signed in event received", args);
  }
  ...
  private async checkNpmVersion() {
    try {
      const res: execResult = await this.red.exec._run(
        NPM_COMMAND,
        ["-v"],
        {},
        false,
  ...
  private onSelectionChanged(
    ...
    this.appMenu!.setMenuItemEnabled(
      "tools.nodegen",
      selection &&
        selection.nodes &&
        selection.nodes[0] &&
  ...
  private async onNgrokConnect() {
    try {
      ...
      this.status.ngrokUrl = await ngrok.connect(ngrokOptions);
